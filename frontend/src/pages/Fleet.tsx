...
import { STATE_META, attentionLevel, batteryColor } from '../lib/format';
...
// pill label per level — translated at call site via useT()
              ...
              <>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-1 rounded-btn border border-border p-0.5">
                    {([
                      { key: 'all' as StatusFilter,         label: t('fleet.filter.all') },
                      { key: 'running' as StatusFilter,     label: t('fleet.filter.running') },
                      { key: 'charging' as StatusFilter,    label: t('fleet.filter.charging') },
                      { key: 'docked' as StatusFilter,      label: t('fleet.filter.docked') },
                      { key: 'maintenance' as StatusFilter, label: t('fleet.filter.maintenance') },
                    ]).map((o) => (
                      <button
                        key={o.key}
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {visible.length === 0 ? (
                    <p className="col-span-full py-8 text-center text-sm text-muted">{t('fleet.noMatch.filters')}</p>
                  ) : visible.map((r) => {
                      ...
                      >
                        ...
                        <div className="mt-3">
                          <div className="mb-1 flex items-center justify-between text-xs">
                            <span className="text-muted">{t('fleet.card.battery')}</span>
            ...
            >
              <Popup minWidth={180}>
                <div style={{ fontFamily: 'system-ui, sans-serif', lineHeight: 1.5 }}>
                  ...
                  <div style={{ fontSize: '12px', marginBottom: '2px' }}>{t('fleet.popup.battery')}&nbsp;: {r.battery}%</div>
                  <div style={{ fontSize: '12px', color: '#888', marginBottom: '8px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '180px' }}>
                    {r.currentMission ?? t('fleet.popup.noMission')}
                  </div>
                  <button
                    type="button"
...
function LiveIndicator({ updatedAt, now }: { updatedAt: Date | null; now: number }) {
  ...
  const label = secs < 60
    ? t('fleet.live.ago.s', { n: secs })
    : t('fleet.live.ago.min', { n: Math.round(secs / 60) });
  return (
...
function SummaryStrip({
  ...
  return (
    <section className="surface-card flex flex-wrap items-center gap-x-6 gap-y-3 px-5 py-3" aria-label={t('fleet.summary.label')}>
      <Stat label={t('fleet.summary.online')} value={`${online}/${total}`} />
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
        {byState.map(({ s, n }) => (
          <span key={s} className="inline-flex items-center gap-1.5 text-sm">
            ...
            <span className="text-muted">{t(`fleet.state.${s}` as Parameters<typeof t>[0])}</span>
      ...
      <button
        ...
        title={attentionOnly ? t('fleet.summary.resetFilter') : t('fleet.summary.filterWatch')}
      >
        <span className="text-xs text-muted">{t('fleet.summary.watch')}</span>
        <span className={`text-base font-semibold tabular-nums ${attention > 0 ? 'text-warning' : ''}`}>{attention}</span>
      </button>
      <Stat label={t('fleet.summary.battery')} value={`${avgBattery}%`} tone={batteryColor(avgBattery)} />
    </section>
  );
}
...
function FleetSkeleton() {
  const t = useT();
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-busy="true" aria-label={t('fleet.skeleton.label')}>
