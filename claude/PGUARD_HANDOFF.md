...
Route guards enforced in routing AND components (never CSS-hidden). `/parametres` and `/gestion` are superadmin+admin only; clients cannot access either.
...
- Superadmin + admin only (top-level route, not nested under `/robots/:id/`)
- Appears in sidebar under Administration section, after Gestion
